...
public class ConfigScriptGenerate
{
    ...
    private static string CONFIG_PATH = Application.dataPath + "/Editor/Config/";

    private static int DATA_ID;
    private static string REGISTER_LIST;
    private static string GETCONFIGLIST;
    ...
    private static void CreateDatabaseScript()
    {
        ...
        for (int i = 0; i < configPaths.Length; i++)
        {
            ...
            GETCONFIGLIST += string.Format("public {0}Data get{1}Data(int nId, bool bEmptyLog = true)\n\t\t{{\n", textAsset.name, textAsset.name);
            GETCONFIGLIST += string.Format("\t\t\tvar data = GetDatabase<{0}Database>().GetDataByKey(nId);\n", textAsset.name);
            GETCONFIGLIST += string.Format("\t\t\tif (bEmptyLog && data == null && nId != int.MaxValue)\n\t\t\t\tDebuger.Log(\"{0} Is Null! ID:\" + nId);\n\t\t\treturn data;\n\t\t}}\n", textAsset.name);
            GETCONFIGLIST += string.Format("\t\tpublic List<{0}Data> getAll{1}Data()\n\t\t{{\n", textAsset.name, textAsset.name);
            GETCONFIGLIST += string.Format("\t\t\treturn GetDatabase<{0}Database>().FindAll();\n\t\t}}\n"
                , textAsset.name);
    ...
    private static string GetCsvSerialize(TextAsset textAsset)
    {
        ...
        for (int i = 0; i < keyCount; i++)
        {
            ...
            {
                csvSerialize += string.Format("m_tempData.{0}=m_datas[i][{1}];\n\t\t\t\t\tm_tempData.{2} = m_tempData.{3}.Replace(\"\\\\n\", \"\\n\");", attributes[1], i, attributes[1], attributes[1]);
            }
            ...
            {
                csvSerialize += string.Format("m_tempData.{0}=CSVConverter.ConvertToArray<long>(m_datas[i][{1}].Trim());", attributes[1], i);
            }
            else if (attributes[0] == "float[]")
            {
            ...
            {
                csvSerialize += string.Format("m_tempData.{0}=CSVConverter.ConvertToArray<double>(m_datas[i][{1}].Trim());", attributes[1], i);
            }
            else if (attributes[0] == "string[][]")
            {
                csvSerialize += string.Format("m_tempData.{0}=CSVConverter.ConvertToMultiArray<string>(m_datas[i][{1}].Trim());", attributes[1], i);
            }
            else if (attributes[0] == "bool[][]")
            {
                csvSerialize += string.Format("m_tempData.{0}=CSVConverter.ConvertToMultiArray<bool>(m_datas[i][{1}].Trim());", attributes[1], i);
            }
            else if (attributes[0] == "int[][]")
            {
                csvSerialize += string.Format("m_tempData.{0}=CSVConverter.ConvertToMultiArray<int>(m_datas[i][{1}].Trim());", attributes[1], i);
            }
            else if (attributes[0] == "float[][]")
            {
                csvSerialize += string.Format("m_tempData.{0}=CSVConverter.ConvertToMultiArray<float>(m_datas[i][{1}].Trim());", attributes[1], i);
            }
            else if (attributes[0] == "double[][]")
            {
                csvSerialize += string.Format("m_tempData.{0}=CSVConverter.ConvertToMultiArray<double>(m_datas[i][{1}].Trim());", attributes[1], i);
            }
            else if (attributes[0] == "long[][]")
            {
                csvSerialize += string.Format("m_tempData.{0}=CSVConverter.ConvertToMultiArray<long>(m_datas[i][{1}].Trim());", attributes[1], i);
            }
    ...
    private static void CreateDatabaseManagerScript()
    {
        ...
        template = template.Replace("$GetConfigList", GETCONFIGLIST);
        GenerateScript("ConfigDataMgr", template);
    }
}
